...
| Capability | Where in the app | Source |
|---|---|---|
| 10-dimension trust score on ~10 datasets | "Portfolio Scorecard" page | `PNC_DQ_RESULTS` (synthetic) |
| Per-dataset dimension drilldown + 90-day trend | "Dataset Detail" page | `PNC_DQ_RESULTS` + registries |
| Live DMF measurements -> 4 dimensions | "Cortex DQ" page | `VW_PNC_DMF_*` (real data, manual SP fallback) |
| Weekly anomaly detection (z-score by L1) | "Workforce Shifts" page | `VW_WORKFORCE_ANOMALIES` (real trended data) |
| Reorg detector (L1/L2 bulk movements) | "Workforce Shifts" page | `VW_REORG_EVENTS` (real trended data) |
| Time-to-fill distribution + recruiter workload | "TA Analytics" page | `VW_TIME_TO_FILL`, `VW_RECRUITER_WORKLOAD` (real position data) |
...
- `app/streamlit_app.py` — all 6 pages, ~1300 lines, single file
